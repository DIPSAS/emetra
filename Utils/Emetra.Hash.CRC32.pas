...
unit Emetra.Hash.CRC32;

interface

  procedure CalcCRC32 ( p:  pointer; const AByteCount:  integer; var ACRCvalue:  cardinal);
  function StrCRC32( const s: string ): cardinal;
...
begin
  ...
  begin
    ACRCvalue := (ACRCvalue SHR 8) XOR Table[ q^ XOR (ACRCvalue AND $000000FF) ];
    INC(q)
  end
  {$WARNINGS ON}
end;
...
begin
  Result := 0;
  CalcCRC32( pChar( s ), Length( s ), Result );
end;
